...
describe("factLines", () => {
  it("groups a fully tagged photograph the way a caption reads", () => {
    const lines = factLines(DEFAULT_OVERLAY_FACTS, { entry, meta: shot });
  ...
  it("drops a group the photograph has nothing to say about", () => {
    ...
    const lines = factLines(DEFAULT_OVERLAY_FACTS, { entry, meta: undefined });
  ...
  it("shows the facts it has when only some are tagged", () => {
    ...
    const lines = factLines(DEFAULT_OVERLAY_FACTS, { entry, meta: partial });
  ...
  it("ignores an id nobody registered, rather than throwing", () => {
    ...
    expect(factLines(["nope", "name"], { entry, meta: shot })[0]?.parts).toEqual([
      "DSC_0178.NEF",
    ]);
  });
});
