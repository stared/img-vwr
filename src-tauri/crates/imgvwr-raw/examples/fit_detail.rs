//! Fits decoder detail settings (NR, sharpening) against sibling camera JPEGs: 1:1 centre patches
//! under a settings grid, aligned and locally brightness-equalised, scoring texture with tone divided out. One JSON line per (file, setting).
...
fn run_one(raw: &Path) -> Result<(), String> {
    ...
    unsafe {
        ...
        // The raw's active area is a few pixels larger than the JPEG's, so centres are matched by relative position.
        ...
        // Staged, not a full cross product: the winners-crossed stage captures the interactions at a quarter of the time.
        ...
        // The camera sharpens edges while smoothing flat areas, so the two are searched together too.
        for sharp in [0.4, 0.7, 1.0] {
            for lnr in [0.35, 0.7] {
                grid.push(Knobs { lnr, cnr: defaults.1, sharp, detail: 0.0 });
            }
        }
        // The camera smooths MORE than our decode from ISO ~1000 up (edge-energy ratio 0.72-0.80), so the top of the NR range is probed too.
        for lnr in [0.85, 1.0] {
...
/// (score, noise_ratio, hf_ratio): mean |Δ| of locally-normalised luma after best integer alignment;
/// noise_ratio = HF energy ours/camera in the flattest quarter (>1 noisier), hf_ratio the same in the busiest (<1 softer).
fn compare(ours: &[f32], cam: &[f32]) -> (f32, f32, f32) {
    ...
    // Flat/busy classified by the camera's gradient alone, so both sums bin identically.
